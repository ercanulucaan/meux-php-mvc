...
<head>
    ...
    <title>@yield('title', 'Hoşgeldiniz') - {{ $site_title }}</title>
    <!-- Tailwind CSS -->
    ...
    <div class="relative pt-6 px-4 sm:px-6 lg:px-8 z-50">
        <nav class="relative flex items-center justify-between sm:h-10 lg:justify-start" aria-label="Global">
            ...
            <div class="hidden md:block md:ml-10 md:space-x-8">
                ...
                @guest
                <a href="{{ route('auth.login') }}" class="font-medium text-blue-600 hover:text-blue-500">Giriş Yap</a>
                @endguest
                @auth
                <a href="{{ route('admin.dashboard') }}" class="font-medium text-blue-600 hover:text-blue-500">Panel</a>
                @endauth
            </div>
        </nav>
    </div>
